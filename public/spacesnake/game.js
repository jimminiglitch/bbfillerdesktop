...
alien1Img.src = "https://cdn.glitch.global/75a5fcf4-9aa2-4423-a1f1-825c09432eac/Alien1.png?v=1747432886886"
alien1Img.crossOrigin = "anonymous"

const alien2Img = new Image()
alien2Img.src = "https://cdn.glitch.global/75a5fcf4-9aa2-4423-a1f1-825c09432eac/Alien2.png?v=1747432896175"
alien2Img.crossOrigin = "anonymous"

const astroBikeImg = new Image()
astroBikeImg.src = "https://cdn.glitch.global/75a5fcf4-9aa2-4423-a1f1-825c09432eac/astrobike.png?v=1747473389333"
...
function triggerAstroBikerAnimation() {
  ...
  // Adjust z-index distribution to favor smaller sizes (negative z values)
  // Use a skewed random distribution that produces more negative values
  astroBiker.z = Math.random() * Math.random() * 8 - 6 // Skewed distribution, mostly -6 to 0, occasionally up to 2

  // Scale now directly based on z-position, but with a smaller range for larger sizes
  astroBiker.scale = 0.3 + Math.max(0, (astroBiker.z + 6) / 8) * 1.2 // Scale between 0.3 and 1.5, mostly on the smaller end

  astroBiker.speed = 2 + Math.random() * 6 // More speed variation (2-8)
  ...
  // More vertical position variation
  astroBiker.y = canvas.height * 0.1 + Math.random() * canvas.height * 0.8

  // Set as active and record start time
...
function updateAliens() {
  ...
  if (alien1.active) {
    ...
    const duration = 5000 // 5 seconds for the full animation (increased from 3s)

    if (elapsed < duration / 2) {
      // Rising up - come up higher into the frame
  ...
  // Update Alien 2 animation with multiple directions
  if (alien2.active) {
    const elapsed = now - alien2.animationStartTime
    const duration = 6000 // 6 seconds for the full animation
  ...
  if (astroBiker.active) {
    ...
    // Add some vertical movement (wave pattern)
    const elapsed = now - astroBiker.animationStartTime
